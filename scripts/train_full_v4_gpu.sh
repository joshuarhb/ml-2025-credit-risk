#!/bin/bash
#SBATCH --job-name=hc_v4_gpu
#SBATCH --output=reports/logs/v4_gpu_%j.out
#SBATCH --error=reports/logs/v4_gpu_%j.err
...
python src/train_full_v4.py --device gpu --tag $TAG
...
if kaggle datasets status $USER_ID/home-credit-model-$SLUG > /dev/null 2>&1; then
    kaggle datasets version -p $UPLOAD_DIR -m "Full Hybrid V4 (GPU)"
else
